...
- ```Без эффектов``` - переход на режим без эффектов рунитов (ВНИМАНИЕ: НЕСОХРАНЕННЫЕ ДАННЫЕ ПРОПАДУТ СРАЗУ, ПОКА НЕ ПОФИКСИЛ)
...
```Выбор класса``` влияет на то, с каким приоритетом и весом определяются те или иные эффекты рунитов

### Ввод рунитов ручной

- ```Тип рунита``` - влиент на то, в какие ядра он может установится
- ```Название рунита``` - влиает на его возможные характеристики (количество очков, заряд, эффекты)
- ```Заряд```- Заряд рунита
- ```Очки```- Очки Хаоса/Порядка рунита
- ```Доп. эффект```-  Эффекты рунита (Доп. урон, Сила атаки и тд)
- ```Уровень```-  Уровень эффекта рунита
...
- Определние типа также происходит автоматически

Очень важно правильно делать скриншоты. Для максимальной точности распознавания необходимо, чтоб скриншот попадали только значения зарядов, очков и эффекты.
